...
my $fallback = sub ($file) { };

method mime-type($file) {
    $file ~~ / (\.<[a..zA..Z0..9]>+) $/ or return;
    $MIME-TYPES{lc $/[0]} || $fallback(lc $/[0]);
}

method add-type(@ext-type) {
    for @ext-type -> $ext, $type {
        $MIME-TYPES{lc $ext} = $type;
    }
}

sub set-fallback(Callable $cb) {
    $fallback = $cb;
}
